...
const Announce = require('./Announce/frameworkMessages')
...
class Pomegranate {
  ...
  shutdown() {
    if(this.stateStep === 'shutdown'){ return }
    this.stateStep = 'shutdown'
    ...
    if(errors.length) {
      // console.log(errors);
      this.SystemLogger.error(`Could not continue due to the following:`)
      _fp.each((e) => {
        this.SystemLogger.error(`${e.ModuleName} has errors.`)
        // Validation Errors, Userland plugin is miss programmed.
        _fp.each((pe) => {
          this.SystemLogger.error(pe.message)
          this.SystemLogger.error(pe.stack || 'No stacktrace available.')
        }, e.ValidationErrors || [])
        // Interface errors, Userland code is broke.
        _fp.each((pe) => {
          this.SystemLogger.error(pe.message)
          this.SystemLogger.error(pe.stack || 'No stacktrace available.')
        }, e.InterfaceErrors || [])
        //Plugin Errors, Usually structural.
        _fp.each((pe) => {
          this.SystemLogger.error(pe.message)
          this.SystemLogger.error(pe.stack || 'No stacktrace available.')
        }, e.PluginErrors || [])
  ...
  cleanUpExit() {
    if(this.stateStep === 'start') {
      ...
      return this.stop()
        .then(() => {
          this.FrameworkEvents.emit('exitReady')
        })
    }
    this.FrameworkEvents.emit('exitReady')
    return this
  ...
  getCommands() {
    return this.PluginIterator.getCommands()
  }
